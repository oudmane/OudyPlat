...
class URL extends Object {
    ...
    public function __construct($url) {
		...
		if($url = parse_url($url)) parent::__construct($url);
		// parse query vars
		if(isset($this->query)) parse_str($this->query, $this->queries);
		// parse paths
		if($this->path) $this->paths = array_slice(explode('/', $this->path), 1);
	}
	/**
	...
	public function inPath($key, $returnNextPath = false) {
		...
		if(!isset($this->paths) || empty($this->paths)) return false;
	...
	public function getURL($secure = null) {
		$url = '';
		if($secure === true) $url .= 'https:';
		else if($secure === false) $url .= 'http:';
